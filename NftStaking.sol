...
contract Staking is ERC721Holder, Ownable {
     ...
     */
    uint256 public constant UID = 0x12ea82e97e97cd54405b116b4209cbaf8bcb22911b5ad1045e81ea6caf7d47fa;
    ...
    /**
     * @dev Emitted every time a token is claimed
     *
     * Emitted in claimReward()
     *
     * @param by address that unstaked the NFT
     * @param time block timestamp the NFT were staked at
     * @param tokenId token ID of NFT that was unstaken
     ...
     *               staking of the NFT
     */
    event RewardClaimed(address indexed by, uint256 indexed tokenId, uint256 time, uint256 reward);
    ...
    function currentRewardsOf(uint256 _tokenId) public view returns (uint256) {
        ...
        uint256 stakingTime = block.timestamp - lastRewardAt[_tokenId];
    ...
    function stake(uint256[] memory _tokenIds) public {
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            lastRewardAt[tokenId] = block.timestamp;
    ...
    /**
     * @notice Send ERC-20 token rewards
     *
     * @dev Sender must have first staken the NFTs
     *
     * @param tokenId token IDs of NFT to claim reward from
     */
    function claimReward(uint256 tokenId) public {
        ...
        // Emit event
        emit RewardClaimed(msg.sender, tokenId, block.timestamp, rewards);

        // Reset the last reward timestamp
        lastRewardAt[tokenId] = block.timestamp;

        // Mint total rewards for all sent NFTs
        // to user
        erc20.mint(msg.sender, rewards);
    }
    ...
    function claimRewardAll() public {
        // Cache the length of the staked tokens set for the owner
        uint256 stakedTokensLength = stakedTokens[msg.sender].length();
        ...
        require(totalRewards > 0, "no reward to claim");
